 ...
 * ```
 * Modified by `swordcube` to add pausing to FlxTween.
 * 
 * Yes `active` is a thing but i'm adding `paused` because what if people wanna stop a tween for a bit
 * but then they pause and unpause and suddenly their tween is active?
 * 
