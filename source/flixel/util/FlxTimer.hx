 ...
 * ```
 * Modified by `swordcube` to add pausing to FlxTimers.
 * 
 * Yes `active` is a thing but i'm adding `paused` because what if people wanna stop a timer for a bit
 * but then they pause and unpause and suddenly their timer is active?
 * 
 * You should still use `active`, `paused` should only be used for the pause menu!
 * 
...
class FlxTimer implements IFlxDestroyable {
     ...
     * Used interally by the game to pause and unpause timers when you pause on unpause in any song.
     */
	...
	public function update(elapsed:Float):Void {
		...
		while ((_timeCounter >= time) && !paused && active && !finished) {
			_timeCounter -= time;
			_loopsCounter++;
...
class FlxTimerManager extends FlxBasic {
	...
	override public function update(elapsed:Float):Void {
		...
		for (timer in _timers) {
            @:privateAccess
			if (!timer.paused && timer.active && !timer.finished && timer.time >= 0) {
	...
	public function completeAll():Void {
		...
		for (timer in _timers)
            @:privateAccess
			if (timer.loops > 0 && !timer.paused && timer.active)
