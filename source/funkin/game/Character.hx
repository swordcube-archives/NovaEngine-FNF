...
class Character extends FNFSprite implements IBeatReceiver {
	/**
	...
	**/
	...
	public function loadPsych(?mod:Null<String>) {
		...
		// JSON Data
		var data:PsychCharacter = Json.parse(Assets.getText(jsonPath));
	...
	public function loadYoshi(?mod:Null<String>) {
		...
		// JSON Data
		var data:YoshiCharacter = Json.parse(Assets.getText(jsonPath));
	...
	public function loadXML(?mod:Null<String>) {
        if(!Paths.exists(Paths.xml('data/characters/$curCharacter/config')))
            return Console.error('Occured on character: $curCharacter | The XML doesn\'t exist!');

		// Load the intial XML Data.
		var xml:Xml = Xml.parse(Assets.getText(Paths.xml('data/characters/$curCharacter/config'))).firstElement();
		if (xml == null)
		...
		switch (atlasType.toLowerCase()) {
			case "packer":
				load(PACKER, Paths.getSparrowAtlas('characters/$spritesheetName'));
			default:
	...
	override function playAnim(anim:String, force:Bool = false, ?context:AnimationContext = NORMAL, reversed:Bool = false, frame:Int = 0) {
		super.playAnim(anim, force, context, reversed, frame);
