...
// had to do this shit because git version of hxCodec stopped working correctly
// great
...
class MP4Handler {
	...
	public function playMP4(path:String, ?repeat:Bool = false, ?outputTo:FlxSprite = null, ?isWindow:Bool = false, ?isFullscreen:Bool = false,
        ...
        bitmap.volume = (FlxG.sound.volume + 0.3) * volume;
	...
	function update(e:Event) {
		if (Init.controls.ACCEPT && skippable) {
			if (bitmap.isPlaying)
				onVLCComplete();
		}

		bitmap.volume = (FlxG.sound.volume + 0.3) * volume; // shitty volume fix. then make it louder.
